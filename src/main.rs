...
async fn main() -> color_eyre::eyre::Result<()> {
	...
	watcher(Api::<EndpointSlice>::all(client.clone()), wc)
		...
		.try_for_each(|endpoint_slice| {
			...
			async move {
				debug!("Starting to look at {}", endpoint_slice.name_any());
				...
				let nais_apps = Api::<DynamicObject>::namespaced_with(client, team_name, &nais_crd);
				match nais_apps.get_opt(app_name).await {
					Err(e) => {
						error!("Error occurred when attempting to fetch `{}/{} {}` from namespace `{}`: {}", &nais_gvk.group, &nais_gvk.version, &nais_gvk.kind, &team_name, e);
					...
					Ok(nais_app) => {
						let Some(_) = nais_app else {
							warn!(
								...
								&nais_gvk.group, &nais_gvk.version, &nais_gvk.kind, team_name
							);
							return Ok(());
						};
					},
