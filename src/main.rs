		...
		.try_for_each(|endpoint_slice| {
			...
			async move {
				let Some(namespace) = endpoint_slice.namespace() else {
					...
					error!("Unable to ascertain namespace of: {}", endpoint_slice.name_any());
					return Ok(());
				};
				info!("Starting to look at: {}/{}", namespace, endpoint_slice.name_any());
				...
				let Some(app_name) = endpoint_slice.labels().get("app") else {
					...
					warn!("Unable to find `app` label on EndpointSlice: {}/{}", namespace, endpoint_slice.name_any());
					return Ok(());
				};
				let Some(team_name) = endpoint_slice.labels().get("team") else {
					// We expect the team's name to be present as this label,
					//  as is customary for things generated by naiserator/NAIS apps
					warn!("Unable to find `team` label on EndpointSlice: {}/{}", namespace, endpoint_slice.name_any());
					return Ok(());
				};
				...
				info!("Ascertained that this EndpointSlice seems to be a product of a NAIS app: {}/{} -> {}/{}", namespace, &app_name, namespace, endpoint_slice.name_any());
